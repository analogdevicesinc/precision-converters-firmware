...
adxl1002_sensitivity = 0.04     # Sensitivity of ADXL1002 accelerometer (40mv/g)
adxl1002_offset_error = 65      # Offset error at constant position (0G)

# Global variables
weight_input = 0
loadcell_offset = [ 0, 0, 0, 0 ]    # For 4 loadcell channels
loadcell_gain = [ 0, 0, 0, 0 ]      # For 4 loadcell channels
...
def init_sensor_measurement():
    ...
    uri = "serial:COM5,230400"  # For UART, baud rate must be same as set in the FW. COM port is physical Or VCOM.
    device_name = "ad4170"      # Name of the device must be same as set in the FW.
    ######################################

    # Create an IIO device context
    device = ad4170_xattr(uri, device_name)
...
def perform_sensor_measurement():
    ...
    while not key_pressed:
        ...
        for chn in device.channel:
            sleep(short_time)
            if (demo_config == 'Accelerometer'):
                adc_raw = chn.raw
                scale = chn.scale
                voltage = (adc_raw * scale) / 1000
                force = (voltage / adxl1002_sensitivity) - adxl1002_offset_error
                result_str = result_str + str(round(force,4)) + 'G  '
            elif (demo_config == 'Loadcell'):
                adc_raw = chn.raw
                try:
                    chn_indx = chn_mappping[chn.name]
