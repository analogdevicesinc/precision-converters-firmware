					...
					withEnv(axisEnv) {
						try {
							stage("Build-${PLATFORM_NAME}-${ACTIVE_DEVICE}-${EVB_INTERFACE}") {
								...
								bat "cd projects/${projectName} & make all LINK_SRCS=n TARGET_BOARD=${PLATFORM_NAME} BINARY_FILE_NAME=${PLATFORM_NAME}-${ACTIVE_DEVICE}-${EVB_INTERFACE} NEW_CFLAGS+=-DACTIVE_DEVICE=${ACTIVE_DEVICE} NEW_CFLAGS+=-D${EVB_INTERFACE}"
