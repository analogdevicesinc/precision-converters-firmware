...
def init_data_capture():
    ...
    uri = "serial:COM6,230400"  # For UART, baud rate must be same as set in the FW. COM port is physical Or VCOM.
    device_name = "ad7193"      # Name of the device must be same as set in the FW.
    ######################################
    ...
    device.rx_enabled_channels = [0]   # Noise test is primarily performed on AIN1-AIN2.
...
def read_buffered_data():
    ...
    adc_ref_voltage_nv = 5000000000.0
    gain = 128
    ...
    for i in range(0, len(data_arr)):
        sample_volt = (data_arr[i] - (offset)) * (adc_ref_voltage_nv / ((adc_max_count_bipolar) * gain))
