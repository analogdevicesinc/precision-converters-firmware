# @file    ad777x_data_capture.py
# @brief   Data capturing for the AD777x device
#
...
def init_data_capture():
    ...
    uri = "serial:COM45,230400"  # For UART, baud rate must be same as set in the FW. COM port is physical Or VCOM.
    device_name = "ad7770"      # Name of the device must be same as set in the FW.
    ######################################

    # Create an IIO device context
    device = ad777x(uri, device_name)
    ...
    # Note: Do not modify the rx_enabled_channels as the firmware has been set to read all channels irrespective of the enabled channels.
