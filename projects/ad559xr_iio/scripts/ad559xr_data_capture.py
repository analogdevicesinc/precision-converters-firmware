...
def init_data_capture():
    ...
    uri = "serial:COM43,230400"  # For UART, baud rate must be same as set in the FW. COM port is physical Or VCOM.
    device_name = "ad5592r"      # Name of the device must be same as set in the FW.
    ######################################

    # Create an IIO device context
    device = ad5592r(uri, device_name)
    device._ctx.set_timeout(100000)

    ######## User configuration ##########
    # Channels to be captured e.g. ["voltage0"]: 1chn, ["voltage0, voltage1"]: 2chns. Configure the channels in the FW to be adc. The channels not configured adc in FW will be ignored.
