...
def runSeqBuild(Map config =[:], projectName) {
	stage("Build-${config.PLATFORM_NAME}-${config.EVB_INTERFACE}") {
		...
		sh "cd projects/${projectName} ; make all LINK_SRCS=n TARGET_BOARD=${config.PLATFORM_NAME} BINARY_FILE_NAME=${config.PLATFORM_NAME}-${config.EVB_INTERFACE} NEW_CFLAGS+=-D${config.EVB_INTERFACE}"
		...
		ws('workspace/pcg-fw') {
			...
			try {
				stage("Test-${config.PLATFORM_NAME}-${config.EVB_INTERFACE}") {
					...
					bat returnStatus: true , script: "mbedflsh --disk ${platformInfo["mountpoint"]} --file ${WORKSPACE}\\projects\\${projectName}\\build\\${config.PLATFORM_NAME}-${config.EVB_INTERFACE}.bin"               
