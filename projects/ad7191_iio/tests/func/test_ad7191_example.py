...
def test_ad719x(serial_port, device_name, serial_com_type, target_reset):
    ...
    # Create a new instance of AD7191 class which creates
    # a device context as well for IIO interface
    ad7191_dev = ad719x(uri_str, iio_device[device_name])

    # *********** Perform data capture check ***********

    # Note: Test is done based on the assumption that, a fixed DC voltage of 1.8v is applied to 
    #       chn0 (AIN1) of AD7191 device configured in unipolar and pseudo-differential mode.
    print("\nData capture test for channel 0 => \n")
    chn = 0
    ad7191_dev._rx_data_type = np.uint32
    ad7191_dev.rx_enabled_channels = [chn]
    ad7191_dev.rx_buffer_size = 100
    raw_data = ad7191_dev.rx()

    data = ndarray((ad7191_dev.rx_buffer_size,),int)
    scale = ad7191_dev.channel[chn].scale
    offset = ad7191_dev.channel[chn].offset
    for indx in range(ad7191_dev.rx_buffer_size):
