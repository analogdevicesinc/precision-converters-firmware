 ...
 *   @details This module contains the configurations needed for ad7191
...
static int32_t init_interrupt(void)
{
	...
	ret = no_os_irq_register_callback(trigger_irq_desc,
					  IRQ_INT_ID, &ext_int_callback_desc);
	if (ret) {
		return ret;
	}

	ret = no_os_irq_trigger_level_set(trigger_irq_desc,
					  IRQ_INT_ID, NO_OS_IRQ_EDGE_FALLING);
	if (ret) {
		return ret;
	}
#endif
...
int32_t init_system(void)
{
	...
	/* Uncomment it out after testing */
