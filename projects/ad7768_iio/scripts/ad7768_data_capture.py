...
def init_data_capture():
    ...
    device_name = "ad7768"      # Name of the device must be same as set in the FW.
    ######################################

    # Create an IIO device context
    device = ad7768(uri, device_name)
    device._ctx.set_timeout(10000000)

    ######## User configuration ##########
    device.rx_enabled_channels = ["voltage0", "voltage1", "voltage2", "voltage3"]
    # The block of samples to be captured. Total samples are received in multiple iterations or blocks
    samples_block = 4000    # The samples needs to be captured in smaller blocks due to limitations
    ...
    device._rx_data_type = np.uint32     # size of ADC sample
    device._rx_stack_interleaved = False
...
def init_data_logger():
    ...
    for chn in range(0, chn_count):
        item = "Ch {}".format(device.rx_enabled_channels[chn])
        row.insert(chn, item)
    writer.writerow(row)
