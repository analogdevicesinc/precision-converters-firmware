					...
					withEnv(axisEnv) {
						try {
							stage("Build-${PLATFORM_NAME}-${EVB_INTERFACE}-${EVB_TYPE}") {
								...
								bat "cd projects/${projectName} & make test TARGET_BOARD=${PLATFORM_NAME} ARTIFACT-NAME=${PLATFORM_NAME}-${EVB_INTERFACE}-${EVB_TYPE} TEST_FLAGS+=-D${EVB_INTERFACE}"
...
def runSeqBuild(Map config =[:], projectName) {
	stage("Build-${config.PLATFORM_NAME}-${config.EVB_INTERFACE}-${config.EVB_TYPE}") {
		...
		bat "cd projects/${projectName} & make test TARGET_BOARD=${config.PLATFORM_NAME} ARTIFACT-NAME=${config.PLATFORM_NAME}-${config.EVB_INTERFACE}-${config.EVB_TYPE} TEST_FLAGS+=-D${config.EVB_INTERFACE}"
