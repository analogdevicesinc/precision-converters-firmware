...
def init_data_capture():
    ...
    uri = "serial:COM16,230400"  # For UART, baud rate must be same as set in the FW. COM port is physical Or VCOM.
    device_name = "ad4112"      # Name of the device must be same as set in the FW.
    ######################################

    # Create an IIO device context
    device = ad717x(uri, device_name)
    device._ctx.set_timeout(100000)

    ######## User configuration ##########
    # Channels to be captured e.g. [voltage0]: 1chn, [voltage0,voltage1]: 2chns, [voltage0,voltage1,voltage2,voltage3]: 4chns
    device.rx_enabled_channels = ["voltage0", "voltage1","voltage2","voltage3","voltage4"]
    ...
    device.rx_buffer_size = samples_block    # Per channel sample count
    device._rx_data_type = np.int32     # size of sample
...
def read_user_inputs():
    ...
    if (samples_count == 0):
        ...
        run_continuous = False
        iterations = (int)(samples_count / device.rx_buffer_size)
        if (samples_count % device.rx_buffer_size):
