def getBuildMatrix(projectName) {
	...
	if (env.CHANGE_TARGET == "main" || env.CHANGE_TARGET == "develop") {
		...
		buildMap = [
			PLATFORM_NAME: ['SDP_K1', 'DISCO_F769NI', 'NUCLEO_L552ZE_Q'],
			ACTIVE_DEVICE: ['DEV_AD7124_4', 'DEV_AD7124_8'],
	...
	List buildMatrix = buildInfo.getMatrixCombination(buildMap).findAll { axis ->
		// Skip 'all platforms except SDP-K1 + NO SDRAM' and
		!(axis['PLATFORM_NAME'] != 'SDP_K1' && axis['SDRAM'] == 'NO_SDRAM') &&
		// Skip 'all platforms except SDP-K1 + VIRTUALCOM' and
		!(axis['PLATFORM_NAME'] != 'SDP_K1' && axis['COM_TYPE'] == 'USE_VIRTUAL_COM_PORT')&&
		// Skip 'all platforms except SDP-K1 + devices other than DEV_AD7124_4'
		!(axis['PLATFORM_NAME'] != 'SDP_K1' && axis['ACTIVE_DEVICE'] != 'DEV_AD7124_4')
	}
...
def doBuild(Map config =[:], projectName) {
	stage("Build-${config.PLATFORM_NAME}-${config.ACTIVE_DEVICE}-${config.COM_TYPE}-${config.SDRAM}") {
		...
		echo "Building for ${config.PLATFORM_NAME} Platform and ${config.ACTIVE_DEVICE} Device with ${config.COM_TYPE} UART type, ${config.SDRAM}"
		echo "^^**^^ ^^**^^ ^^**^^ ^^**^^ ^^**^^ ^^**^^ ^^**^^ ^^**^^ ^^**^^ ^^**^^"
...
def doTest(Map config =[:], projectName) {
	node(label : "${hw.getAgentLabel(platform_name: config.PLATFORM_NAME, evb_ref_id: 'EVAL-'+ config.ACTIVE_DEVICE)}") {
		checkout scm
		try {
			...
			// Fetch the stashed files from build stage
				...
				echo "^^**^^ ^^**^^ ^^**^^ ^^**^^ ^^**^^ ^^**^^ ^^**^^ ^^**^^ ^^**^^ ^^**^^"                       
				echo "Running on node: '${env.NODE_NAME}'"
				echo "Testing for ${config.PLATFORM_NAME} Platform and ${config.ACTIVE_DEVICE} Device with ${config.COM_TYPE} UART type, ${config.SDRAM}"
				echo "^^**^^ ^^**^^ ^^**^^ ^^**^^ ^^**^^ ^^**^^ ^^**^^ ^^**^^ ^^**^^ ^^**^^"
				platformInfo = hw.getPlatformInfo(platform_name: config.PLATFORM_NAME, evb_ref_id: 'EVAL-' + config.ACTIVE_DEVICE)
