...
class ADIStream(object):
    ...
    def __init__(
        ...
        self.stream._ctx.set_timeout(10000000)
        ...
        self.wave_list.extend(["sine"] * (self.chn_cnt - len(self.wave_list)))
        ...
        for val in range(self.chn_cnt):
            if self.chn_cnt == 1:
                if self.wave_list[val] == "pwm":
                    self.data = eval(
                        "gen_obj.gen_pwm_wave(duty_cycle = " + str(self.duty_cyc / 100) + ")"
                    )
                else:
                ...
                if self.wave_list[val] == "pwm":
                    self.data.append(
                        eval("gen_obj.gen_pwm_wave(duty_cycle = " + str(self.duty_cyc / 100) + ")")
                    )
                else:
                    self.data.append(
...
if __name__ == "__main__":
    ...
    if args["test_device"]:
        ...
        if device_name not in supported_devices:
            raise Exception(f"The device {device_name} is not supported. Supported devices are"
    ...
    if args["output_freq"] is not None and args["output_freq"] <= 0:
        raise argparse.ArgumentTypeError('output frequency required cannot be a negative number')

    if args["neg_voltage_ref"] >= args["pos_voltage_ref"]:
        raise argparse.ArgumentTypeError('negative reference cannot be greater than the positive reference')
