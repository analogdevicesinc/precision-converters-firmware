...
static int32_t init_gpio(void)
{
	...
	no_os_gpio_remove(dclk_desc);
	no_os_gpio_remove(odr_desc);
	no_os_gpio_remove(dout0_desc);
	no_os_gpio_remove(dout1_desc);
	return -EINVAL;
#endif
...
static int32_t init_interrupt(void)
{
	do {
		...
		if (no_os_irq_set_priority(external_int_desc, IRQ_INT_ID, 0) != 0) {
			break;
		}
	...
	no_os_irq_ctrl_remove(external_int_desc);
	return -EINVAL;
