/***************************************************************************//**
 *   @file    ad4134_iio.c
 *   @brief   Implementation of AD4134 IIO apllication interfaces
********************************************************************************
...
/* IIO trigger name */
#define AD4134_IIO_TRIGGER_NAME		"ad4134_iio_trigger"
...
static struct iio_desc *p_ad4134_iio_desc;

/* AD4134 IIO hw trigger descriptor */
static struct iio_hw_trig *ad4134_hw_trig_desc;

/* Pointer to the struct representing the AD4134 IIO device */
struct ad4134_dev *p_ad4134_dev_inst = NULL;
...
static int attr_offset_val[AD4134_NUM_CHANNELS] = { 0x0 };
 ...
 * structure from ad4134_trigger_handler() */
struct iio_device_data *ad4134_iio_dev_data;

/* Pointer to the ADC data buffer */
static uint8_t *ad4134_dma_buff;
		       ...
		       intptr_t id)
{
	...
	if (ad4134_perform_conv_and_read_sample((uint32_t)channel->ch_num,
						&adc_data_raw) == 0) {
...
{
	if ((reg > AD4134_REG_TEMPERATURE_DATA) ||
	    (ad4134_spi_reg_read(dev, reg, (uint8_t *)readval) != 0)) {
		return -EINVAL;
	}
...
{
	if ((reg > AD4134_REG_TEMPERATURE_DATA) ||
	    (ad4134_spi_reg_write(dev, reg, writeval) != 0)) {
		return -EINVAL;
	}
...
static struct iio_channel iio_ad4134_channels[] = {
	{
		.name = "voltage0",
		.ch_type = IIO_VOLTAGE,
		.channel = 0,
 ...
 */
static int32_t iio_ad4134_prepare_transfer(void *dev_instance,
		uint32_t chn_mask)
{
	int32_t ret;
	...
	ad4134_configure_intr_priority();
#endif // INTERFACE_MODE
	ret = iio_trig_enable(ad4134_hw_trig_desc);
	if (ret) {
		return ret;
	}
#if (INTERFACE_MODE == TDM_MODE)
	ret = start_tdm_dma_to_cb_transfer(ad4134_tdm_desc, ad4134_iio_dev_data,
 ...
 * @return 0 in case of success or negative value otherwise
 */
static int32_t ad4134_read_burst_data_tdm(uint32_t nb_of_bytes,
		struct iio_device_data *iio_dev_data)
{
	uint32_t ad4134_buff_available_size;
	uint32_t timeout;
	...
	do {
		...
		ret = no_os_cb_prepare_async_write(iio_dev_data->buffer->buf, nb_of_bytes,
						   &ad4134_dma_buff, &ad4134_buff_available_size);
		if (ret) {
			return ret;
		}

		/* Trigger TDM-DMA read to capture data into buffer in the background */
		ret = no_os_tdm_read(ad4134_tdm_desc, ad4134_dma_buff,
		...
		ret = no_os_tdm_stop(ad4134_tdm_desc);
		if (ret) {
			return ret;
		}
 ...
 * @return 0 in case of success or negative value otherwise
 */
static int32_t ad4134_read_burst_data_bit_bang(uint32_t nb_of_samples,
		struct iio_device_data *iio_dev_data)
{
	...
	while (sample_index < nb_of_samples) {
		for (ch_id = 0; ch_id < AD4134_NUM_CHANNELS; ch_id++) {
			/* Read ADC data using GPIO Bit banging method for
			 * detecting a level change in ODR and DCLK signals */
			ret = ad4134_perform_conv_and_read_sample(ch_id, &adc_data);
			if (ret) {
				return ret;
			}
...
/**
 * @brief Read buffer data corresponding to AD4134 IIO device
 * @param iio_dev_data[in] - Pointer to IIO device data structure
 * @return 0 in case of success or negative value otherwise
 */
static int32_t iio_ad4134_submit_buffer(struct iio_device_data *iio_dev_data)
{
...
#if (INTERFACE_MODE == TDM_MODE)
	ret = ad4134_read_burst_data_tdm(iio_dev_data->buffer->size, iio_dev_data);
	if (ret) {
		return ret;
	}
#else
	...
	ret = ad4134_read_burst_data_bit_bang(nb_of_samples, iio_dev_data);
	if (ret) {
		return ret;
	}
#endif // INTERFACE_MODE
 ...
 * @return 0 in case of success or negative value otherwise
 */
static int32_t iio_ad4134_end_transfer(void *dev)
{
	int32_t ret;

#if (DATA_CAPTURE_MODE == CONTINUOUS_DATA_CAPTURE)
#if (INTERFACE_MODE != TDM_MODE)
	ret = iio_trig_disable(ad4134_hw_trig_desc);
	if (ret) {
		return ret;
	}
#else // TDM_MODE
 ...
 * @note This function is executed only in case of continuous capture
		 in SPI Mode.
 */
int32_t ad4134_trigger_handler(struct iio_device_data *iio_dev_data)
{
	uint16_t adc_data[AD4134_NUM_CHANNELS] = { 0x0 };
	int32_t ret;
	 ...
	 * global ad4134_iio_dev_data structure variable for future IIO CB operations */
	ret = iio_trig_disable(ad4134_hw_trig_desc);
	if (ret) {
		return ret;
	}

	ad4134_iio_dev_data = iio_dev_data;
#else
	if (!buf_size_updated) {
	...
	ret = ad4134_read_all_channels(adc_data);
	if (ret) {
		return ret;
	}
	...
	ret = no_os_cb_write(iio_dev_data->buffer->buf, &adc_data,
			     BYTES_PER_SAMPLE * AD4134_NUM_CHANNELS);
	if (ret) {
		return ret;
	}

	/* Enable back the interrupt */
	iio_trig_enable(ad4134_hw_trig_desc);
	if (ret) {
		return ret;
	}
#endif
 ...
 */
static int32_t iio_ad4134_init(struct iio_device **desc)
{
	struct iio_device *iio_ad4134_inst;

	iio_ad4134_inst = calloc(1, sizeof(struct iio_device));
	if (!iio_ad4134_inst) {
		return -ENOMEM;
	}

	iio_ad4134_inst->num_ch = sizeof(iio_ad4134_channels) / sizeof(
					  iio_ad4134_channels[0]);
	iio_ad4134_inst->channels = iio_ad4134_channels;
	iio_ad4134_inst->attributes = global_attributes;
	iio_ad4134_inst->debug_reg_read = debug_reg_read;
	iio_ad4134_inst->debug_reg_write = debug_reg_write;
	iio_ad4134_inst->pre_enable = iio_ad4134_prepare_transfer;
	iio_ad4134_inst->submit = iio_ad4134_submit_buffer;
	iio_ad4134_inst->post_disable = iio_ad4134_end_transfer;
#if (DATA_CAPTURE_MODE == CONTINUOUS_DATA_CAPTURE)
	iio_ad4134_inst->trigger_handler = ad4134_trigger_handler;
#endif

	*desc = iio_ad4134_inst;

	return 0;
}

/**
 * @brief Initialization of AD4134 IIO hardware trigger specific parameters
 * @param desc[in,out] - IIO hardware trigger descriptor
 * @return 0 in case of success, negative error code otherwise
 */
static int32_t ad4134_iio_trigger_param_init(struct iio_hw_trig **desc)
{
	struct iio_hw_trig_init_param ad4134_hw_trig_init_params;
	...
	ad4134_hw_trig_init_params.irq_id = IRQ_INT_ID;
	ad4134_hw_trig_init_params.name = AD4134_IIO_TRIGGER_NAME;
	ad4134_hw_trig_init_params.irq_trig_lvl = NO_OS_IRQ_EDGE_FALLING;
	ad4134_hw_trig_init_params.irq_ctrl = external_int_desc;
	ad4134_hw_trig_init_params.cb_info.event = NO_OS_EVT_GPIO;
	ad4134_hw_trig_init_params.cb_info.peripheral = NO_OS_GPIO_IRQ;
	ad4134_hw_trig_init_params.cb_info.handle = trigger_gpio_handle;
	ad4134_hw_trig_init_params.iio_desc = p_ad4134_iio_desc;

	/* Initialize hardware trigger */
	ret = iio_hw_trig_init(&hw_trig_desc, &ad4134_hw_trig_init_params);
	if (ret) {
		return ret;
	}
...
/**
 * @brief Initialize the AD4134 device for iio interface.
 * @return 0 in case of success, negative error code otherwise
 */
int32_t ad4134_iio_initialize(void)
{
	...
	struct iio_device *p_iio_ad4134_dev;

#if (DATA_CAPTURE_MODE == CONTINUOUS_DATA_CAPTURE)
	static struct iio_trigger ad4134_iio_trig_desc = {
		.is_synchronous = true,
	};

	/* IIO trigger init parameters */
	static struct iio_trigger_init iio_trigger_init_params = {
		.descriptor = &ad4134_iio_trig_desc,
		.name = AD4134_IIO_TRIGGER_NAME,
	};
#endif
	...
	/* Initialize AD4134 device and peripheral interface */
	init_status = ad4134_init(&p_ad4134_dev_inst, &ad4134_init_params);
	if (init_status) {
		return init_status;
	}
	...
	if (hw_mezzanine_is_valid) {
		...
		init_status = iio_ad4134_init(&p_iio_ad4134_dev);
		if (init_status) {
			return init_status;
		}
		...
		iio_device_init_params[0].dev = p_ad4134_dev_inst;
		iio_device_init_params[0].dev_descriptor = p_iio_ad4134_dev;
	...
	init_status = iio_init(&p_ad4134_iio_desc, &iio_init_params);
	if (init_status) {
		return init_status;
	}
	...
	init_status = ad4134_iio_trigger_param_init(&ad4134_hw_trig_desc);
	if (init_status) {
		return init_status;
	}
#endif
	...
	/* Initialize the data capture interface for AD4134 */
	init_status = ad4134_data_capture_init(p_ad4134_dev_inst);
	if (init_status) {
		return init_status;
	}
