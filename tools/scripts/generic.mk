...
APPEND_CONTENT_TO_FILE = echo $1 >> $2
...
PROJECT_OBJECT_FILES_NAMES = $(BUILD_DIRECTORY)/object-files-names.txt
PROJECT_INCLUDE_PATHS_NAMES = $(BUILD_DIRECTORY)/include-path-names.txt
...
#################### Rule for Putting include paths names in text file ###################
PHONY_TARGET += build-prerequisites
build-prerequisites:
	$(MUTE_COMMAND) $(foreach d,$(ALL_INCLUDE_PATHS),$(call APPEND_CONTENT_TO_FILE,$d,$(PROJECT_INCLUDE_PATHS_NAMES)) $(command_separator)) echo. $(HIDE_OUTPUT)
...
$(BUILD_DIRECTORY)/%.o: $$(call FULL_PATH_OF_FILE, %).cpp | $$(@D)/.
	...
	$(MUTE_COMMAND) $(CPP) $(CPP_FLAGS) @$(PROJECT_INCLUDE_PATHS_NAMES)  $< -o $@

$(BUILD_DIRECTORY)/%.o: $$(call FULL_PATH_OF_FILE, %).c | $$(@D)/.
	$(call print_line,[Compiling] $(notdir $<))
	$(MUTE_COMMAND) $(CC) $(C_FLAGS) @$(PROJECT_INCLUDE_PATHS_NAMES)  $< -o $@

##################### Creating Elf File ########################################
ifeq 'mbed' '$(PLATFORM_NAME)'
$(PROJECT_ELF_FILE): $(OBJECTS_FILES_IN_BUILD) $(MBED_STATIC_LIBRARY) $(PROJECT_LINKER_SCRIPT)
	$(MUTE_COMMAND) $(foreach d,$(RELATIVE_OBJECTS_FILES_IN_BUILD),$(call APPEND_CONTENT_TO_FILE,$d,$(PROJECT_OBJECT_FILES_NAMES)) $(command_separator)) echo. $(HIDE_OUTPUT)
	$(MUTE_COMMAND) cd $(ROOT_DIR_PATH) $(command_separator) $(CC) $(LD_FLAGS) -T $(PROJECT_LINKER_SCRIPT) -o $(PROJECT_ELF_FILE) @$(PROJECT_OBJECT_FILES_NAMES) @$(GENERATED_ARCHIVE_FILE) $(LINKER_LIBRARIES)
endif
...
$(PROJECT_ELF_FILE_FOR_TEST): $(OBJECTS_FILES_IN_BUILD) $(MBED_STATIC_LIBRARY) $(PROJECT_LINKER_SCRIPT)
	$(MUTE_COMMAND) $(foreach d,$(RELATIVE_OBJECTS_FILES_IN_BUILD),$(call APPEND_CONTENT_TO_FILE,$d,$(PROJECT_OBJECT_FILES_NAMES)) $(command_separator)) echo. $(HIDE_OUTPUT)
	$(MUTE_COMMAND) cd $(ROOT_DIR_PATH) $(command_separator) $(CC) $(LD_FLAGS) -T $(PROJECT_LINKER_SCRIPT) -o $(PROJECT_ELF_FILE_FOR_TEST) @$(PROJECT_OBJECT_FILES_NAMES) @$(GENERATED_ARCHIVE_FILE) $(LINKER_LIBRARIES)
endif
