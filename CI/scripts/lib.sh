...
for script in $COMMON_SCRIPTS ; do
	[ ! -f "CI/scripts/$script" ] || continue
	[ ! -f "build/$script" ] || continue
	mkdir -p build
	ensure_command_exists wget
	wget https://raw.githubusercontent.com/analogdevicesinc/precision-converters-firmware/main/CI/scripts/$script \
		-O build/$script
done
